...
BOOL CGvisR2R_LaserDoc::LoadWorkingInfo()
{
	...
	if (0 < ::GetPrivateProfileString(_T("System"), _T("Use3Layer"), NULL, szData, sizeof(szData), sPath))
		WorkingInfo.System.bUse3Layer = _ttoi(szData) ? TRUE : FALSE;
	else
		WorkingInfo.System.bUse3Layer = TRUE;
	...
	if (0 < ::GetPrivateProfileString(_T("Last Job"), _T("Process Unit Code"), NULL, szData, sizeof(szData), sPath))
		WorkingInfo.LastJob.sProcessNum = CString(szData);//m_sProcessNum
	else
	{
		AfxMessageBox(_T("Process Code가 설정되어 있지 않습니다."), MB_ICONWARNING | MB_OK);
		WorkingInfo.LastJob.sProcessNum = CString(_T("VS90"));//m_sProcessNum
	}
	...
	if (0 < ::GetPrivateProfileString(_T("Last Job"), _T("Engrave ItsUp Code"), NULL, szData, sizeof(szData), sPath))
		WorkingInfo.LastJob.sEngItsUpCode = CString(szData);//m_sItsCode = m_sOrderNum = 
	else
		WorkingInfo.LastJob.sEngItsUpCode = _T("");//m_sItsCode = m_sOrderNum = 

	if (0 < ::GetPrivateProfileString(_T("Last Job"), _T("Engrave ItsDn Code"), NULL, szData, sizeof(szData), sPath))
		WorkingInfo.LastJob.sEngItsDnCode = CString(szData);//m_sItsCode = m_sOrderNum = 
	else
		WorkingInfo.LastJob.sEngItsDnCode = _T("");//m_sItsCode = m_sOrderNum = 
	...
	m_sTestOrderNum = WorkingInfo.LastJob.sEngraveOrderNum;//m_sItsCode = m_sOrderNum = 
...
BOOL CGvisR2R_LaserDoc::GetAoiInfoUp(int nSerial, int *pNewLot, BOOL bFromBuf) // TRUE: CHANGED, FALSE: NO CHANGED 
{
	...
	if (WorkingInfo.LastJob.sLotUp != Status.PcrShare[0].sLot || WorkingInfo.LastJob.sEngItsUpCode != Status.PcrShare[0].sItsCode)
	{
		bUpdate = TRUE;
		WorkingInfo.LastJob.sLotUp = Status.PcrShare[0].sLot;
		 WorkingInfo.LastJob.sEngItsUpCode = Status.PcrShare[0].sItsCode;//m_sItsCode =
	}
...
BOOL CGvisR2R_LaserDoc::GetAoiInfoDn(int nSerial, int *pNewLot, BOOL bFromBuf) // TRUE: CHANGED, FALSE: NO CHANGED 
{
	...
	if (WorkingInfo.LastJob.sLotDn != Status.PcrShare[1].sLot || WorkingInfo.LastJob.sEngItsDnCode != Status.PcrShare[1].sItsCode)
	{
		bUpdate = TRUE;
		WorkingInfo.LastJob.sLotDn = Status.PcrShare[1].sLot;
		WorkingInfo.LastJob.sEngItsDnCode = Status.PcrShare[1].sItsCode;//m_sItsCode = 
	}
...
int CGvisR2R_LaserDoc::LoadPCRUp(int nSerial, BOOL bFromShare)	// return : 2(Failed), 1(정상), -1(Align Error, 노광불량), -2(Lot End)
{
	...
	if (WorkingInfo.LastJob.sLotUp != strLot || WorkingInfo.LastJob.sEngItsUpCode != sItsCode)
	{
		bUpdate = TRUE;
		WorkingInfo.LastJob.sLotUp = strLot;
		WorkingInfo.LastJob.sEngItsUpCode = sItsCode;//m_sItsCode = 
	}
...
int CGvisR2R_LaserDoc::LoadPCRDn(int nSerial, BOOL bFromShare)	// return : 2(Failed), 1(정상), -1(Align Error, 노광불량), -2(Lot End)
{
	...
	if (WorkingInfo.LastJob.sLotDn != strLot || WorkingInfo.LastJob.sEngItsDnCode != sItsCode)
	{
		bUpdate = TRUE;
		WorkingInfo.LastJob.sLotDn = strLot;
		WorkingInfo.LastJob.sEngItsDnCode = sItsCode;//m_sItsCode = 
	}
...
void CGvisR2R_LaserDoc::SetModelInfoProcessNum()
{
	...
	CString sData, sPath = PATH_WORKING_INFO;
	sData = WorkingInfo.LastJob.sProcessNum;//m_sProcessNum
...
void CGvisR2R_LaserDoc::SetCurrentInfo()
{
	...
	::WritePrivateProfileString(_T("Infomation"), _T("ItsUp Code"), sData, sPath);
	sData = WorkingInfo.LastJob.sLotUp;//m_sLotNum;
	::WritePrivateProfileString(_T("Infomation"), _T("Current LotUp"), sData, sPath);
	sData = WorkingInfo.LastJob.sProcessNum;//m_sProcessNum
...
void CGvisR2R_LaserDoc::GetCurrentInfo()
{
	...
	if (0 < ::GetPrivateProfileString(_T("Infomation"), _T("ItsUp Code"), NULL, szData, sizeof(szData), sPath))
		WorkingInfo.LastJob.sEngItsUpCode = CString(szData);//m_sItsCode
	else
		WorkingInfo.LastJob.sEngItsUpCode = _T("");//m_sItsCode

	if (0 < ::GetPrivateProfileString(_T("Infomation"), _T("Current LotUp"), NULL, szData, sizeof(szData), sPath))
		WorkingInfo.LastJob.sLotUp = CString(szData); //m_sLotNum
	else
		WorkingInfo.LastJob.sLotUp = _T(""); //m_sLotNum

	if (0 < ::GetPrivateProfileString(_T("Infomation"), _T("Current LotDn"), NULL, szData, sizeof(szData), sPath))
		WorkingInfo.LastJob.sLotDn = CString(szData); //m_sLotNum
	else
		WorkingInfo.LastJob.sLotDn = _T(""); //m_sLotNum

	if (0 < ::GetPrivateProfileString(_T("Infomation"), _T("Process Unit Code"), NULL, szData, sizeof(szData), sPath))
		WorkingInfo.LastJob.sProcessNum = CString(szData);//m_sProcessNum
	else
		WorkingInfo.LastJob.sProcessNum = _T("");//m_sProcessNum
...
void CGvisR2R_LaserDoc::CheckCurrentInfo()
{
	...
	if (0 < ::GetPrivateProfileString(_T("Infomation"), _T("ItsUp Code"), NULL, szData, sizeof(szData), strFolder))
	{
		strTemp = CString(szData);
		if (strTemp != WorkingInfo.LastJob.sEngItsUpCode)//m_sItsCode
		{
			WorkingInfo.LastJob.sEngItsUpCode = strTemp;//m_sItsCode = m_sOrderNum
		}
	}
...
void CGvisR2R_LaserDoc::SetEngItsUpCode(CString sItsCode)
{
	pDoc->WorkingInfo.LastJob.sEngItsUpCode = sItsCode;//pDoc->m_sItsCode = 
	::WritePrivateProfileString(_T("Last Job"), _T("Engrave ItsUp Code"), pDoc->WorkingInfo.LastJob.sEngItsUpCode, PATH_WORKING_INFO);
}

void CGvisR2R_LaserDoc::SetEngItsDnCode(CString sItsCode)
{
	pDoc->WorkingInfo.LastJob.sEngItsDnCode = sItsCode;//pDoc->m_sItsCode = 
	::WritePrivateProfileString(_T("Last Job"), _T("Engrave ItsDn Code"), pDoc->WorkingInfo.LastJob.sEngItsDnCode, PATH_WORKING_INFO);
}
...
BOOL CGvisR2R_LaserDoc::MakeLayerMappingHeader()
{
	...
	Path[2] = WorkingInfo.LastJob.sEngItsUpCode; //pDoc->m_sItsCode;
	Path[3] = WorkingInfo.LastJob.sLotUp; //pDoc->m_sLotNum;
	...
	if (bDualTest)
	{
		if (pDoc->WorkingInfo.LastJob.sLayerUp.IsEmpty() || pDoc->WorkingInfo.LastJob.sLayerDn.IsEmpty())//pDoc->m_sLayerUp pDoc->m_sLayerDn
			return FALSE;
	}
	else
	{
		if (pDoc->WorkingInfo.LastJob.sLayerUp.IsEmpty())//pDoc->m_sLayerUp
			return FALSE;
	}

	sName.Format(_T("%s.txt"), Path[3]);// pDoc->m_sLotNum);
	...
	::WritePrivateProfileString(_T("Infomation"), _T("Process Unit Code"), pDoc->WorkingInfo.LastJob.sProcessNum, sPath);//m_sProcessNum
	::WritePrivateProfileString(_T("Infomation"), _T("Current Model"), pDoc->WorkingInfo.LastJob.sModelUp, sPath);//pDoc->m_sModelUp
	::WritePrivateProfileString(_T("Infomation"), _T("ItsUp Code"), WorkingInfo.LastJob.sEngItsUpCode, sPath);//pDoc->m_sItsCode
	::WritePrivateProfileString(_T("Infomation"), _T("Current LotUp"), WorkingInfo.LastJob.sLotUp, sPath);//pDoc->m_sLotNum
	::WritePrivateProfileString(_T("Infomation"), _T("Current Layer Up"), pDoc->WorkingInfo.LastJob.sLayerUp, sPath);//pDoc->m_sLayerUp

	if (bDualTest)
		::WritePrivateProfileString(_T("Infomation"), _T("Current Layer Dn"), pDoc->WorkingInfo.LastJob.sLayerDn, sPath); //pDoc->m_sLayerDn
	else
		::WritePrivateProfileString(_T("Infomation"), _T("Current Layer Dn"), _T(""), sPath);

	::WritePrivateProfileString(_T("Infomation"), _T("Last ItsUp Serial"), pDoc->WorkingInfo.LastJob.sEngItsUpCode, sPath);//pDoc->m_sLayerUp
...
BOOL CGvisR2R_LaserDoc::MakeLayerMappingSerial(int nIdx, int nItsSerial)
{
	...
	::WritePrivateProfileString(sItsSerail, _T("Process Unit Code"), WorkingInfo.LastJob.sProcessNum, sPath);//m_sProcessNum
	::WritePrivateProfileString(sItsSerail, _T("Current Model"), WorkingInfo.LastJob.sModelUp, sPath);//pDoc->m_sModelUp
	::WritePrivateProfileString(sItsSerail, _T("ItsUp Code"), WorkingInfo.LastJob.sEngItsUpCode, sPath);//pDoc->m_sItsCode
	::WritePrivateProfileString(sItsSerail, _T("Current LotUp"), WorkingInfo.LastJob.sLotUp, sPath);//pDoc->m_sLotNum
	::WritePrivateProfileString(sItsSerail, _T("Current Layer Up"), WorkingInfo.LastJob.sLayerUp, sPath);//pDoc->m_sLayerUp

	if (bDualTest)
		::WritePrivateProfileString(sItsSerail, _T("Current Layer Dn"), WorkingInfo.LastJob.sLayerDn, sPath);//pDoc->m_sLayerDn
	else
...
BOOL CGvisR2R_LaserDoc::SetItsSerialInfo(int nItsSerial)
{
	...
	Path[0] = pDoc->WorkingInfo.System.sPathOldFile;
	Path[1] = pDoc->WorkingInfo.LastJob.sModelUp;//pDoc->m_sModelUp;
	Path[2] = WorkingInfo.LastJob.sEngItsUpCode;// pDoc->m_sItsCode;
	Path[3] = WorkingInfo.LastJob.sLotUp;//pDoc->m_sLotNum;
	Path[4] = pDoc->WorkingInfo.LastJob.sLayerUp;//pDoc->m_sLayerUp;
	Path[5] = pDoc->WorkingInfo.LastJob.sLayerDn;//pDoc->m_sLayerDn;
	...
	sName.Format(_T("%s.txt"), WorkingInfo.LastJob.sEngItsUpCode);//pDoc->m_sItsCode
	sPath.Format(_T("%s%s\\%s\\%s"), Path[0], Path[1], Path[2], sName); // ITS_Code.txt
...
BOOL CGvisR2R_LaserDoc::GetItsSerialInfo(int nItsSerial, BOOL &bDualTest, CString &sLot, CString &sLayerUp, CString &sLayerDn, int nOption)		// 내층에서의 ITS 시리얼의 정보
{
	...
	Path[0] = pDoc->WorkingInfo.System.sPathOldFile;
	Path[1] = pDoc->WorkingInfo.LastJob.sModelUp;
	Path[2] = pDoc->WorkingInfo.LastJob.sEngItsUpCode;// pDoc->m_sItsCode;

	sName.Format(_T("%s.txt"), Path[2]);//pDoc->m_sItsCode
	sPath.Format(_T("%s%s\\%s\\%s"), Path[0], Path[1], Path[2], sName); // ITS_Code.txt
...
CString CGvisR2R_LaserDoc::GetItsFolderPath()
{
	...
	Path[1] = pDoc->WorkingInfo.LastJob.sModelUp;
	Path[2] = pDoc->WorkingInfo.LastJob.sEngItsUpCode;
...
CString CGvisR2R_LaserDoc::GetItsPath(int nSerial, int nLayer)	// RMAP_UP, RMAP_DN, RMAP_INNER_UP, RMAP_INNER_DN
{
	...
	{
	case RMAP_UP: // 외층 Top
		str.Format(_T("%s_L1_%04d_T_%s_%s_AVR01_%s.dat"), pDoc->WorkingInfo.LastJob.sEngItsUpCode, nSerial, WorkingInfo.LastJob.sSelUserName, WorkingInfo.System.sMcName, sTime);//m_sItsCode
		sPath.Format(_T("%s\\Outer\\%s"), sItsFolderPath, str);
		break;
	case RMAP_DN: // 외층 Bottom
		str.Format(_T("%s_L4_%04d_B_%s_%s_AVR01_%s.dat"), pDoc->WorkingInfo.LastJob.sEngItsDnCode, nSerial, WorkingInfo.LastJob.sSelUserName, WorkingInfo.System.sMcName, sTime);//m_sItsCode
		sPath.Format(_T("%s\\Outer\\%s"), sItsFolderPath, str);
		break;
	case RMAP_INNER_UP: // 내층 Top
		str.Format(_T("%s_L2_%04d_T_%s_%s_AVR01_%s.dat"), pDoc->WorkingInfo.LastJob.sEngItsUpCode, nSerial, WorkingInfo.LastJob.sSelUserName, WorkingInfo.System.sMcName, sTime);//m_sItsCode
		sPath.Format(_T("%s\\Outer\\%s"), sItsFolderPath, str);
		break;
	case RMAP_INNER_DN: // 내층 Bottom
		str.Format(_T("%s_L3_%04d_B_%s_%s_AVR01_%s.dat"), pDoc->WorkingInfo.LastJob.sEngItsDnCode, nSerial, WorkingInfo.LastJob.sSelUserName, WorkingInfo.System.sMcName, sTime);//m_sItsCode
		sPath.Format(_T("%s\\Outer\\%s"), sItsFolderPath, str);
		break;
	}
