...
void CGvisR2R_LaserView::Eng1PtDoMarking()
{
	...
	if (m_bEngSt)
	{
		...
		case ENG_ST + (Mk1PtIdx::DoMk) :				// Mk 마킹 시작
			...
			{
				if (!SetMdxLotAndShotNum(pDoc->WorkingInfo.LastJob.sEngItsUpCode, nLastSerialNext))//pDoc->m_sItsCode
				{
					Buzzer(TRUE, 0);
					TowerLamp(RGB_RED, TRUE);
					EngStop(TRUE);
			...
			if (IsRun())
			{
				if (Is2dReadDone())
				{
					if (!pDoc->WorkingInfo.System.bNoMk)
					{
						...
						if (m_sGetItsCode != pDoc->WorkingInfo.LastJob.sEngItsUpCode)//pDoc->m_sItsCode
						{
							pView->SetErrorRead2dCode(_PcId::_Engrave);
							sMsg.Format(_T("설정된 ITS CODE : %s \r\n각인된 ITS CODE : \n\r일치하지 않습니다."), pDoc->WorkingInfo.LastJob.sEngItsUpCode, m_sGetItsCode);//pDoc->m_sItsCode
							pView->MsgBox(sMsg);
						}
					}
					else
