...
void CDlgUtil02::AtDlgShow()
{
	...
	GetDlgItem(IDC_LOT_EDIT)->SetWindowText(pDoc->WorkingInfo.LastJob.sLotUp);
	GetDlgItem(IDC_ITS_EDIT)->SetWindowText(pDoc->WorkingInfo.LastJob.sEngItsUpCode);
	GetDlgItem(IDC_EDIT_PROCESS_NUM)->SetWindowText(pDoc->WorkingInfo.LastJob.sProcessNum);
...
{
	//UpdateData(TRUE);
	m_ModelComboUp.GetWindowText(pDoc->WorkingInfo.LastJob.sModelUp);
	m_LayerComboUp.GetWindowText(pDoc->WorkingInfo.LastJob.sLayerUp);
	GetDlgItem(IDC_LOT_EDIT)->GetWindowText(pDoc->WorkingInfo.LastJob.sLotUp);
	GetDlgItem(IDC_ITS_EDIT)->GetWindowText(pDoc->WorkingInfo.LastJob.sEngItsUpCode);
	GetDlgItem(IDC_EDIT_PROCESS_NUM)->GetWindowText(pDoc->WorkingInfo.LastJob.sProcessNum);

	if (Is3Layer(pDoc->WorkingInfo.LastJob.sModelUp))//pDoc->m_sModelUp
	{
		GetDlgItem(IDC_LOT_EDIT2)->GetWindowText(pDoc->WorkingInfo.LastJob.sLotDn);
		GetDlgItem(IDC_ITS_EDIT2)->GetWindowText(pDoc->WorkingInfo.LastJob.sEngItsDnCode);
	}
	else
	{
		pDoc->WorkingInfo.LastJob.sLotDn = pDoc->WorkingInfo.LastJob.sLotUp;
		pDoc->WorkingInfo.LastJob.sEngItsDnCode = pDoc->WorkingInfo.LastJob.sEngItsUpCode;
	}
...
void CDlgUtil02::ModifyLayerComboData(CComboBox *pComboBox, int nAoi)
{
	...
	//if (nAoi == 0)
	{
		Dir = pDoc->WorkingInfo.System.sPathCamSpecDir + pDoc->WorkingInfo.LastJob.sModelUp + _T("\\*.ini");//pDoc->m_sModelUp
	}
	//else
	//{
...
void CDlgUtil02::OnSelchangeLayerComboUp()
{
	...
	m_LayerComboUp.GetLBText(nIndex, pDoc->WorkingInfo.LastJob.sLayerUp);//pDoc->m_sLayerUp
...
void CDlgUtil02::OnSelchangeLayerComboDn()
{
	...
	m_LayerComboDn.GetLBText(nIndex, pDoc->WorkingInfo.LastJob.sLayerDn);//pDoc->m_sLayerDn
...
BOOL CDlgUtil02::Is3Layer(CString sModel)
{
...
BOOL CDlgUtil02::Is3LayerInner(CString sModel)
{
	CString sParsingChar9;
	sParsingChar9 = sModel.Mid(8, 1);
	sParsingChar9.MakeUpper();
...
BOOL CDlgUtil02::Is3LayerOutter(CString sModel)
{
	CString sParsingChar9;
	sParsingChar9 = sModel.Mid(8, 1);
	sParsingChar9.MakeUpper();
	if (sParsingChar9 == "C" && pDoc->GetTestMode() == MODE_OUTER)
		return TRUE;
	return FALSE;
}

void CDlgUtil02::Enable3LayerInner(BOOL bEnable)
{
