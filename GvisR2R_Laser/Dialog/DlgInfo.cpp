...
BEGIN_MESSAGE_MAP(CDlgInfo, CDialog)
	...
	ON_BN_CLICKED(IDC_CHK_USE_3LAYER, &CDlgInfo::OnBnClickedChkUse3layer)
	ON_STN_CLICKED(IDC_STC_228, &CDlgInfo::OnStnClickedStc228)
END_MESSAGE_MAP()
...
void CDlgInfo::InitBtn()
{
	...
	myBtn[25].SubclassDlgItem(IDC_CHK_USE_AOI_DUAL_ITS, this); //WORK_MODE : MODE_NONE
	myBtn[25].SetHwnd(this->GetSafeHwnd(), IDC_CHK_USE_AOI_DUAL_ITS);
	myBtn[25].SetBtnType(BTN_TYPE_CHECK);

	myBtn[26].SubclassDlgItem(IDC_CHK_USE_AOI_DUAL_2D_ITS, this); //WORK_MODE : MODE_LASER
	myBtn[26].SetHwnd(this->GetSafeHwnd(), IDC_CHK_USE_AOI_DUAL_2D_ITS);
	myBtn[26].SetBtnType(BTN_TYPE_CHECK);

	myBtn[27].SubclassDlgItem(IDC_CHK_USE_3LAYER, this);
	myBtn[27].SetHwnd(this->GetSafeHwnd(), IDC_CHK_USE_3LAYER);
	...
	for(i=0; i<MAX_INFO_BTN; i++)
	{
		if(0 == i || 12 == i || 13 == i || 14 == i  || 23 == i || 24 == i || 25 == i || 26 == i || 27 == i)
		{
		...
		if(0 != i && 12 != i && 13 != i && 14 != i && 15 != i && 16 != i && 23 != i && 24 != i && 25 != i && 26 != i && 27 != i)
		{
			myBtn[i].SetFont(_T("굴림체"),16,TRUE);
			myBtn[i].SetTextColor(RGB_BLACK);
...
void CDlgInfo::InitStcTitle()
{
	...
	myStcTitle[62].SubclassDlgItem(IDC_STC_16, this); //ITSUP Label
	myStcTitle[63].SubclassDlgItem(IDC_STC_19, this); //ITSDN Label
...
void CDlgInfo::InitStcData()
{
	...
	myStcData[16].SubclassDlgItem(IDC_STC_17, this); // ITSUp코드
	myStcData[17].SubclassDlgItem(IDC_STC_228, this); // ITSDn코드
...
void CDlgInfo::Disp()
{
	...
	myStcData[16].SetText(pDoc->WorkingInfo.LastJob.sEngItsUpCode);	// IDC_STC_17	ITSUp코드
	myStcData[17].SetText(pDoc->WorkingInfo.LastJob.sEngItsDnCode);	// IDC_STC_228	ITSDn코드
...
{
	CString sItsUpCode, sItsDnCode;

	switch (nID)
	{
	case IDD_DLG_UTIL_02:
		GetDlgItem(IDC_STC_17)->GetWindowText(sItsUpCode);
		GetDlgItem(IDC_STC_228)->GetWindowText(sItsDnCode);

		CDlgUtil02 dlg;
		dlg.DoModal();
		//pDoc->m_sItsCode = pDoc->m_sLotNum;
		       
		if (sItsUpCode != pDoc->WorkingInfo.LastJob.sEngItsUpCode)//pDoc->m_sItsCode
		{
			GetDlgItem(IDC_STC_17)->SetWindowText(pDoc->WorkingInfo.LastJob.sEngItsUpCode); // IDC_ITS_EDIT pDoc->m_sItsCode
		}
		       
		if (sItsDnCode != pDoc->WorkingInfo.LastJob.sEngItsDnCode)//pDoc->m_sItsCode
		{
...
void CDlgInfo::OnStnClickedStc17()
{
	...
	pDoc->SetEngItsUpCode(sData);

#ifdef USE_ENGRAVE
	if (pView && pView->m_pEngrave)
		pView->m_pEngrave->SetEngItsUpCode();	//_ItemInx::_EngItsCode
#endif
}
...
void CDlgInfo::ChkUse3layer()
{
	...
	if (bOn)
	{
		...
		sData = pDoc->WorkingInfo.System.bUseDualIts ? _T("1") : _T("0");
		::WritePrivateProfileString(_T("System"), _T("UseDualIts"), sData, PATH_WORKING_INFO);
		pDoc->SetMkInfo(_T("Signal"), _T("UseDualIts"), pDoc->WorkingInfo.System.bUseDualIts);			// 3Layer검사

		sData = pDoc->WorkingInfo.System.bUseDual2dIts ? _T("1") : _T("0");
		::WritePrivateProfileString(_T("System"), _T("UseDual2dIts"), sData, PATH_WORKING_INFO);
		pDoc->SetMkInfo(_T("Signal"), _T("UseDual2dIts"), pDoc->WorkingInfo.System.bUseDual2dIts);		// 3Layer검사
	}
	else
	{
		pDoc->WorkingInfo.System.bUse3Layer = FALSE;
	}

	sData = pDoc->WorkingInfo.System.bUse3Layer ? _T("1") : _T("0");
	::WritePrivateProfileString(_T("System"), _T("Use3Layer"), sData, PATH_WORKING_INFO);
	pDoc->SetMkInfo(_T("Signal"), _T("Use3Layer"), pDoc->WorkingInfo.System.bUse3Layer);		// 3Layer검사
}


void CDlgInfo::OnStnClickedStc228()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	...
	CPoint pt;	CRect rt;
	GetDlgItem(IDC_STC_228)->GetWindowRect(&rt);
	pt.x = rt.right; pt.y = rt.bottom;
	ShowKeypad(IDC_STC_228, pt, TO_BOTTOM | TO_RIGHT);

	myStcData[16].SetBkColor(RGB_WHITE);
	myStcData[16].RedrawWindow();

	CString sData;
	GetDlgItem(IDC_STC_228)->GetWindowText(sData);
	pDoc->SetEngItsDnCode(sData);
